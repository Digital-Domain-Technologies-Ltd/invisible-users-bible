...
We'd built a web that worked well for one specific type of user: someone with good vision, working on a desktop, with focused attention and plenty of time. Everyone else had been struggling quietly for years. Now AI agents were struggling loudly, and there was finally commercial pressure to fix the problems. This isn't limited to the web—it applies whenever machines interact, and to the problems people have with real-world objects. So when you read "website" in these books, think bigger.

I want to be clear from the start: I'm not writing this book to criticize AI systems or to point out edge cases and gotcha prompts. I genuinely admire what these technologies have achieved - generating coherent, contextually appropriate text across an extraordinary range of domains. The statistical pattern-matching that makes this possible represents a remarkable engineering accomplishment. My focus isn't on what's wrong with AI agents, but on what we can do to make them work better. When we provide well-structured inputs - semantic HTML, explicit metadata, clear state management - and when AI creators implement appropriate guardrails, the results improve dramatically for everyone. Hallucinations decrease. Citations become accurate. Commerce transactions complete successfully. This book is about recognizing that better-structured websites produce better outcomes, not just for AI agents, but for users with disabilities, search engines, and all web visitors.
